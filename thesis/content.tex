...
\chapter{Matlab WPS}
	...
	\section{Architecture}
		...
		\lstinputlisting[float=!htb,caption={WebSocket opening handshake using a HTTP upgrade request \citep{ietf:rfc6455}.},label={lst:websocket}]{listings/websocket-handshake.txt}
...
\section{Protocol}
	...
	This causes the streaming process to stop accepting inputs, to process all not yet processed inputs, and to request a last potential output from the delegate (\hyperref[fig:sd:swps]{step~13~and~14}), which is forwarded to all listening clients (\hyperref[fig:sd:swps]{step~15}). After this, it destructs the delegate (\hyperref[fig:sd:swps]{steps~16~and~17}) and notifies all registered listeners, that no further outputs will become available by forwarding the stop message (\hyperref[fig:sd:swps]{step~18}) to the clients. The streaming process will destroy itself after this.
	...
	\section{Messages}
		...
		\subsection{Describe \& Description Message}
			Describe messages are directly adopted from the \ac{WPS} Describe Process operation. Due to conditions described in section \ref{sec:stream:processdescription} a client needs to be able to retrieve a description from a running streaming process. The message simply contains the identifier of the process the clients wants to have the description from. An example for this process can be seen in Listing \ref{lst:streaming:message:describe}).
			...
			The reply resembles a \emph{DescribeProcess} response and is encoded in a description message referencing the describe message and containing the streaming process description and (see Listing \ref{lst:streaming:message:description}).
	...
	\begin{figure}
		...
		\end{subfigure}
		\begin{subfigure}{\textwidth}
			\caption{As the dependencies of $fib(4)$ and $fib(5)$ are fulfilled, they can directly be calculated. $fib(7)$ is still missing $fib(6)$.}
		...
		\end{subfigure}
		\caption{Calculation of the seventh Fibonacci number using the Streaming WPS and a simple addition WPS process as delegate.}
	\end{figure}
	\begin{itemize}
		\item feature complete but prototypical
		\item Server:
		\begin{itemize}
			\item based on the 52°North WPS
			\item module, that can be included
			\item besides framework to develope all kinds of streaming processes
			\begin{itemize}
				\item e.g. stateful
			\end{itemize}
			\item default implementation uses another WPS process as delegate
			\item is able to turn every appropriate algortihm into a streaming variant
			\item e.g. Douglas–Peucker, etc.
			\item generic process implementation, that accepts \dots
			\begin{itemize}
				\item a reference to another process (URL and process description)
				\item a list of static inputs, that will be used across process execution
				\item returns process identifier and websocket endpoint
			\end{itemize}
			\item due to stateless WPS, only can create stateless streaming processes
		\end{itemize}
		\item Client
		\begin{itemize}
